...
function paintAdminClients(){if(!$('adminClients'))return;const q=($('adminClientSearch')?.value||'').trim().toLowerCase(),sort=$('adminClientSort')?.value||'name-asc',view=$('adminClientView')?.value||'active';let data=ADMIN_CLIENTS.filter(c=>(view==='all'||(view==='active'?c.active!==false:c.active===false))&&(!q||`${c.business_name||''} ${c.nombre||''} ${c.email||''} ${c.phone||''} ${c.city||''}`.toLowerCase().includes(q)));data=[...data];if(sort==='name-desc')data.sort((a,b)=>String(b.business_name||b.nombre||b.email||'').localeCompare(String(a.business_name||a.nombre||a.email||''),'es'));else if(sort==='recent')data.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));else data.sort((a,b)=>String(a.business_name||a.nombre||a.email||'').localeCompare(String(b.business_name||b.nombre||b.email||''),'es'));$('adminClients').innerHTML=data.map(c=>`<article class="admin-client-card"><div class="admin-client-main"><div class="client-avatar">${esc((c.business_name||c.nombre||c.email||'C').trim().charAt(0).toUpperCase())}</div><div class="client-copy"><div class="row client-title-row"><h3>${esc(c.business_name||c.nombre||c.email||'Cliente')}</h3><span class="status client-status ${c.active===false?'client-inactive':''}">${c.active===false?'Desactivado':'Activo'}</span></div>${c.nombre&&c.business_name?`<p class="client-person">${esc(c.nombre)}</p>`:''}<div class="client-meta">${c.email?`<span>✉ ${esc(c.email)}</span>`:''}${c.phone?`<span>☎ ${esc(c.phone)}</span>`:''}${c.city?`<span>⌖ ${esc(c.city)}</span>`:''}${Number(c.discount_percent||0)>0?`<span>Descuento ${Number(c.discount_percent)}%</span>`:''}</div></div></div><div class="client-actions">${c.active===false?`<button class="btn btn-light" onclick="reactivateClient('${c.id}')">Reactivar cliente</button>`:`<button class="btn btn-danger" onclick="deleteClient('${c.id}','${esc(String(c.business_name||c.nombre||c.email||'Cliente')).replace(/'/g,'&#39;')}')">Eliminar cliente</button>`}</div></article>`).join('')||'<div class="empty">No se encontraron clientes.</div>';}
...
async function reactivateClient(id){if(!(await requireUser('admin')))return;try{await db.collection('usuarios').doc(id).update({active:true,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});ADMIN_CLIENTS=ADMIN_CLIENTS.map(c=>c.id===id?{...c,active:true}:c);paintAdminClients();}catch(e){alert('No se pudo reactivar el cliente: '+(e?.message||e));}}
